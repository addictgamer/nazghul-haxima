...
- [~] Implement spell system parity (`spells.scm` + reagents behavior) *(spell registry is now loaded from `spells.scm` (plus tutorial aliases), with data-driven circles/reagents/context and generic targeted/self-cast resolution wired into cast/cycle/targeting; cast-time context restrictions plus scripted heal/ward/light/locate/unlock/quickness handling are active (including turn-based buff duration tracking and HUD effect visibility); broader source-authentic scripted spell families still pending)*.
